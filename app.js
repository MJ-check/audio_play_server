...
// 音乐相关API
// 获取所有音乐信息
app.get("/api/all_music", (req, res) => api.all_music(pool, req, res));
// 获取最新上传的音乐信息
app.get("/api/last_music", (req, res) => api.last_music(pool, req, res));
// 获取单个音乐信息
app.get("/api/music", (req, res) => api.music(pool, req, res));
// 获取音乐在所有收藏夹内的状态
app.get("/api/status", (req, res) => api.status(pool, req, res));
// 添加音乐进入收藏夹
app.post("/api/add", (req, res) => api.add(pool, req, res));
// 把音乐移除收藏夹
app.post("/api/remove", (req, res) => api.remove(pool, req, res));
// 上传音乐文件
app.post(
  "/api/upload_music", 
  multer({ storage: config.storage_for_upload_music }).single("music_file"), 
  (req, res) => api.upload_music(pool, req, res)
);
// 上传音乐图片 || 更新音乐图片
app.post(
  "/api/upload_music_image",
  multer({ storage: config.storage_for_upload_music_image }).single("music_image"),
  (req, res) => api.upload_music_image(pool, req, res)
);
// 收藏夹相关API
// 获取所有收藏夹信息
app.get("/api/collect_list", (req, res) => api.collect_list(pool, req, res));
// 获取收藏夹内音乐信息
app.get("/api/list", (req, res) => api.list(pool, req, res));
// 创建收藏夹
app.post("/api/new_list", (req, res) => api.new_list(pool, req, res));
// 更新收藏夹图片
app.post(
  "/api/upload_list_image",
  multer({ storage: config.storage_for_upload_list_image }).single("list_image"),
  (req, res) => api.upload_list_image(pool, req, res)
);
app.post("/api/update_list_msg", (req, res) => api.update_list_msg(pool, req, res));
